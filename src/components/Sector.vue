...
const { parentToWorld, cameraMatrix } = useMatrices(positionProp);
const { parseColor } = useColors();
const active = defineModel("active", { default: false });
usePointerIntersection(active, (point) => {
  const b = Vector2.wrap(props.position).transform(cameraMatrix.value);
  const a = Vector2.fromPolar(from.value, props.radius)
    .add(props.position)
    .transform(cameraMatrix.value)
  const c = Vector2.fromPolar(to.value, props.radius)
    .add(props.position)
    .transform(cameraMatrix.value);
